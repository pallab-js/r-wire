...
<div class="flex flex-col h-full" style="background-color: var(--cursor-cream); color: var(--cursor-dark); font-family: var(--font-mono);">
  <div class="p-2 border-b sticky top-0 z-10 shrink-0" style="background-color: var(--surface-200); border-color: var(--border-primary);">
    <div class="relative flex items-center border rounded-sm transition-colors focus-within:border-[var(--border-medium)]" style="background-color: var(--surface-100); border-color: var(--border-primary);">
      ...
      />
      {#if filterText}
        <button on:click={() => filterText = ''} class="bg-transparent border-none cursor-pointer px-2 cursor-btn-ghost" style="color: rgba(38, 37, 30, 0.55);" title="Clear filter">
  ...
  <div class="flex-1 overflow-y-auto p-2 text-sm">
    {#if !layers || layers.length === 0}
      <div class="flex h-full items-center justify-center italic" style="color: rgba(38, 37, 30, 0.55);">No protocol layers available</div>
    {:else}
            ...
            <div class="group">
              ...
              >
                <svg
                  ...
                  style="color: rgba(38, 37, 30, 0.55); transform: {expandedState[i] ? 'rotate(90deg)' : 'rotate(0deg)'}"
                >
                  <polyline points="9 18 15 12 9 6"></polyline>
                </svg>
                <strong class="font-medium" style="color: var(--color-success);">{layer.name}</strong>
              </button>

              {#if expandedState[i]}
                <div class="ml-[22px] border-l pl-2 my-0.5 flex flex-col gap-0.5" style="border-color: var(--border-primary);">
                  {#each filteredFields as field}
                    ...
                    >
                      <span class="w-[180px] shrink-0 font-medium" style="color: var(--cursor-dark);">{field.name}:</span>
                      <span class="break-all flex-1" style="color: rgba(38, 37, 30, 0.55);">{field.value}</span>
                      {#if field.expert}
                        <span class="ml-2 px-1.5 py-0.5 text-xs rounded font-bold uppercase" style="background-color: rgba(207, 45, 86, 0.15); color: var(--color-error);" title={field.expert}>EXPERT</span>
                      {/if}
                      <button
