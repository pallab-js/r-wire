<script lang="ts">
  import type { ForensicNarrative } from '../stores';

  export let narrative: ForensicNarrative;
</script>
...
>
  <div class="max-w-3xl">
    <div class="mb-8">
      ...
      <p
        class="text-xl leading-relaxed font-medium"
        style="color: var(--cursor-dark); font-family: var(--font-serif); font-feature-settings: 'cswh';"
      >
        {narrative.summary}
      </p>
    </div>

    {#if narrative.technical_details.length > 0}
      <div>
        <h3
        ...
        <div class="flex flex-col gap-3">
          {#each narrative.technical_details as detail}
            <div
              class="flex items-start gap-3 p-4 rounded border"
              style="background-color: var(--surface-200); border-color: var(--border-primary);"
            >
              <div class="mt-1" style="color: var(--color-success);">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 24 24"
