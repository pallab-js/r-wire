...
<div
  class="flex flex-col h-full p-6 overflow-hidden"
...
>
  <div class="mb-6 flex items-center justify-between">
    <div>
      ...
      <p class="text-xs" style="color: rgba(38, 37, 30, 0.55);">
        Temporal distribution of packets in this conversation
      </p>
    </div>
    {#if flowPackets.length > 0}
      <div
        class="text-xs px-3 py-1 rounded border"
        style="background-color: var(--surface-200); border-color: var(--border-primary); color: var(--cursor-dark); font-family: var(--font-mono);"
      >
  ...
  <div class="flex-1 flex flex-col min-h-0">
    {#if loading}
      <div
        class="flex-1 flex items-center justify-center italic"
    ...
    {:else if flowPackets.length === 0}
      <div
        ...
        style="color: rgba(38, 37, 30, 0.55);"
      >
        No flow data available for this packet.
      </div>
    {:else}
      <div
        class="relative h-24 rounded-lg border mb-8 overflow-hidden"
        style="background-color: var(--surface-200); border-color: var(--border-primary);"
      >
        <!-- Time scale -->
        <div
          class="absolute inset-x-0 bottom-0 h-6 border-t flex justify-between px-2 items-center text-xs font-mono"
          style="border-color: var(--border-primary); background-color: var(--surface-200); color: rgba(38, 37, 30, 0.55);"
        >
          <span>0ms</span>
        ...
        {#each flowPackets as p}
          ...
          >
            <div
              class="hidden group-hover:block absolute -top-2 left-1/2 -translate-x-1/2 text-[var(--cursor-dark)] text-xs px-1 rounded font-bold z-10 whitespace-nowrap"
      ...
      <div class="flex-1 overflow-y-auto pr-2">
        <h4
          class="text-xs font-bold uppercase mb-3 tracking-wider"
          style="color: rgba(38, 37, 30, 0.55);"
        >
          Event Log
        </h4>
        <div class="flex flex-col gap-1 text-xs" role="list">
          {#each flowPackets as p}
            <button
              class="flex items-center gap-4 py-1.5 px-3 rounded transition-colors border-b cursor-pointer text-left"
              style="border-color: var(--border-primary); background: transparent;"
            ...
            >
              ...
              <span class="w-24" style="color: var(--color-success);"
                >+{((p.timestamp - minTime) / 1000000).toFixed(3)} ms</span
              >
              <span class="w-24" style="color: rgba(38, 37, 30, 0.55);"
                >{p.source_addr === flowPackets[0].source_addr ? 'SRC -> DST' : 'DST -> SRC'}</span
              >
